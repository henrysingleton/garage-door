...
@pytest.mark.parametrize(
    ...
    [
        ...
        # 0. Closed
        ((0, 1), State.CLOSED, State.CLOSED, 0.0, State.CLOSED),
        # 1. Open
        ((1, 0), State.OPEN, State.OPEN, 0.0, State.OPEN),
        # 2. Closing
        ((0, 0), State.CLOSING, State.CLOSING, 0.0, State.CLOSING),
        # 3. Opening
        ((0, 0), State.OPENING, State.OPENING, 0.0, State.OPENING),

        ## Regular transition states

        # 4. Closed -> Opening
        ((0, 0), State.CLOSED, State.CLOSING, 0.0, State.OPENING),
        # 5. Open -> Closing
        ((0, 0), State.OPEN, State.OPENING, 0.0, State.CLOSING),
        # 6. Closing -> Closed
        ((0, 1), State.CLOSING, State.OPEN, 0.0, State.CLOSED),
        # 7. Opening -> Open
        ((1, 0), State.OPENING, State.CLOSED, 0.0, State.OPEN),

        ## Unusual transition states

        # 8. Open -> Closed
        ((0, 1), State.OPEN, State.OPENING, 0.0, State.CLOSED),
        # 9. Closed -> Open
        ((1, 0), State.CLOSED, State.CLOSING, 0.0, State.OPEN),
        ...
        # ((0, 0), State.OPEN, State.OPENING, 0.0, State.OPEN),
    ]
)
